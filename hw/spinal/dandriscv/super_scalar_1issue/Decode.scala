...
case class Decode() extends Component {
  ...
  lsu_micro_op.src2_is_imm := False
...
case class DecodeStage() extends Component {
  ...
  val en_rob_ports = master(Stream(EnROB(PC_WIDTH)))
  ...
  val en_rob_stream = Stream(EnROB(PC_WIDTH))

  // ==================== connect =============================
  decode.pc    := src_stream.pc
  decode.instr := src_stream.instruction

  // ==================== output =============================
  en_rob_stream.valid        := src_stream.valid
  en_rob_stream.pc           := src_stream.pc
  en_rob_stream.pc_next      := src_stream.pc_next
  en_rob_stream.bpu_pred_taken := src_stream.bpu_pred_taken
  en_rob_stream.micro_op     := decode.rob_micro_op
  en_rob_stream.rd_addr      := decode.rd_addr
  en_rob_stream.exception    := decode.exception
  en_rob_stream.rs1_ren      := decode.rs1_ren
  en_rob_stream.rs2_ren      := decode.rs2_ren
  en_rob_stream.rs1_addr     := decode.rs1_addr
  en_rob_stream.rs2_addr     := decode.rs2_addr
  en_rob_stream.imm          := decode.imm
  en_rob_stream.bju_micro_op := decode.bju_micro_op
  en_rob_stream.alu_micro_op := decode.alu_micro_op
  en_rob_stream.lsu_micro_op := decode.lsu_micro_op
  src_stream.ready           := en_rob_stream.ready

  // difftest
  if(DIFFTEST){
    en_rob_stream.instruction := src_stream.instruction
  }

  en_rob_stream >-> en_rob_ports
}
