...
case class SuperScalar() extends Component {
  ...
  val icache = new ITCM(icache_config, icache_axi_config)
  // val bpu = new gshare_predictor(gshare_config)
  val bpu = new static_predictor(gshare_config)
  val decode = new DecodeStage()
  val rob = new ReorderBuffer()
  
  val isq_bju_0 = new IssueQueue(iq_bju_config)
  val isq_alu_0 = new IssueQueue(iq_alu_config)
  val isq_alu_1 = new IssueQueue(iq_alu_config)
  val isq_lsu_0 = new IssueQueue(iq_lsu_config)
  val bju_0 = new BJU()
  val alu_0 = new ALU()
  val alu_1 = new ALU()
  ...
  decode.stall       := False
  decode.src_ports   <> fetch.dst_ports.throwWhen(change_flow)

  // ================= ROB ===============
  rob.en_rob           <> decode.en_rob_ports.throwWhen(change_flow)
