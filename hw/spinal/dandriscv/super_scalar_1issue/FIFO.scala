...
case class FIFO[T <: Data](dataType: T, DEPTH : Int = 2, READ_NEXT : Boolean = false) extends Component{
  ...
  // count empty space of FIFO
  val empty_entry_cnt = Reg(UInt(PTR_WIDTH+1 bits)) init(DEPTH)
  val empty_entry_cnt_next = UInt(PTR_WIDTH+1 bits)
  when(ports.s_ports.fire && !ports.m_ports.fire){
    empty_entry_cnt_next := empty_entry_cnt - 1
  }
  .elsewhen(!ports.s_ports.fire && ports.m_ports.fire){
    empty_entry_cnt_next := empty_entry_cnt + 1
  }
  .otherwise{
    empty_entry_cnt_next := empty_entry_cnt
  }
  when(flush){
    empty_entry_cnt := U(DEPTH)
  }
  .otherwise{
  ...
  has_space := empty_entry_cnt_next > U(1)
