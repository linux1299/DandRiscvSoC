...
case class IssueQueue(p : IssueQueueConfig) extends Component{
  ...
  val entry = new Area{
    ...
    val pc_next = ((IQ_Type=="BJU") || DIFFTEST) generate Reg(UInt(PC_WIDTH bits)) init(0)
    val bpu_pred_taken = (IQ_Type=="BJU") generate RegInit(False)
  ...
  when(en_queue.fire){
    ...
    if(IQ_Type=="BJU"){
      entry.pc_next     := en_queue.pc_next
      entry.bpu_pred_taken := en_queue.bpu_pred_taken
    }
  ...
  if(IQ_Type=="LSU"){
    when(en_queue.fire){
      entry.src2_vld  := en_queue.src2_vld
      entry.src2_data := en_queue.src2_data
    }
    // when wake up, write all equal entry
  ...
  if(IQ_Type=="LSU"){
    de_queue.valid := entry.busy && entry.src1_vld && entry.src2_vld && 
                     (entry.micro_op.lsu_is_store ? (rob_head_ptr===entry.rd_rob_ptr) | True)
  }
  else{
    de_queue.valid := entry.busy && entry.src1_vld && entry.src2_vld
  }
  ...
  if(IQ_Type=="BJU"){
    de_queue.pc_next := entry.pc_next
    de_queue.bpu_pred_taken := entry.bpu_pred_taken
  }
  if(DIFFTEST){
