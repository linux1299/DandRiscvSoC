...
case class ICache(p : ICacheConfig) extends Component{
  ...
  val next_level_bank_addr = cpu_addr_d1(setRange)
  ...
  for(wayId <- 0 until wayCount){
    ...
    sram_banks_data(wayId) := (cpu_bank_index_d1===U(0)) ? sram(wayId*bankNum).ports.rsp.data | sram(wayId*bankNum+1).ports.rsp.data
    ...
    for (bankId <- 0 until bankNum){
      ...
      when(is_hit && U(wayId)===hit_id){ // when hit, read sram
        sram(wayId*bankNum + bankId).ports.cmd.addr  := cpu_bank_addr
      ...
      .elsewhen(next_level_done && U(wayId)===evict_id_miss){ // when read next level data done, read data from victim way
        sram(wayId*bankNum + bankId).ports.cmd.addr  := cpu_bank_addr_d1
        sram(wayId*bankNum + bankId).ports.cmd.valid := True
        sram(wayId*bankNum + bankId).ports.cmd.wen   := False
        sram(wayId*bankNum + bankId).ports.cmd.wdata := B(0, bankWidth bits)
        sram(wayId*bankNum + bankId).ports.cmd.wstrb := B(0, bankWidth/8 bits)
      ...
      .elsewhen(next_level.rsp.valid && U(wayId)===evict_id_miss){ // when read miss, read next level data, write to banks
        sram(wayId*bankNum + bankId).ports.cmd.addr  := (next_level_bank_addr ## U(0, 3 bits)).asUInt + 
                                                        next_level_data_cnt.value.resize(bankDepthBits bits)
        sram(wayId*bankNum + bankId).ports.cmd.valid := True
...
case class ITCM(val config : ICacheConfig, val axiConfig : Axi4Config) extends Component {
  ...
  if(!config.directOutput ){
    ...
    icacheReader.ar.id := U(0)
    icacheReader.ar.len := U(0)
    icacheReader.ar.size := U(0)
    icacheReader.ar.burst := B(1) // INCR
    icacheReader.ar.addr := U(0)
    // r channel tie 1
    icacheReader.r.ready := True

    val sram = new Sram(32, 15) // 4 byte * (2^15) = 128KB

    sram.ports.cmd.valid := icache_ports.cmd.valid
    sram.ports.cmd.addr := icache_ports.cmd.addr(16 downto 2)
    sram.ports.cmd.wen := False
    sram.ports.cmd.wdata := B(0)
    sram.ports.cmd.wstrb := B(0)
    icache_ports.cmd.ready := True

    icache_ports.rsp.valid := sram.ports.rsp.valid
    icache_ports.rsp.data := sram.ports.rsp.data

    // BinTools.initRam(sram.mem, "/home/lin/oscpu/bin/tests/coremark/coremark-riscv64-nemu.bin", false)
  }
  else {
    // connect icache and cpu ports
    icache_ports.cmd <> icacheMaster.cmd
