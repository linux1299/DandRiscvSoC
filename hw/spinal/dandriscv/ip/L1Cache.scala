...
case class L1CacheConfig(cacheSize : Int,
                        bytePerLine : Int,
                        wayCount : Int,
                        addressWidth : Int,
                        cpuDataWidth : Int,
...
case class L1CacheNextLevelCmd(p : L1CacheConfig) extends Bundle{
...
case class L1CacheNextLevelRsp(p : L1CacheConfig) extends Bundle{
  val data = Bits(p.busDataWidth bits)
}
case class L1CacheNextLevelPorts(p : L1CacheConfig) extends Bundle with IMasterSlave{
  val cmd = Stream(L1CacheNextLevelCmd(p))
  val rsp = Flow(L1CacheNextLevelRsp(p))
...
// ==================== L1Cache =============================
case class L1Cache(p : L1CacheConfig) extends Component{
  import p._

  val flush = in Bool()
  val cpu = slave(L1CacheAccess(addressWidth, cpuDataWidth))
  val sram = for(i<-0 until bankNum) yield new Area{
    val ports = master(SramPorts(bankDepthBits, bankWidth))
  }
  val next_level = master(L1CacheNextLevelPorts(p))
  ...
  for(wayId <- 0 until wayCount){
    ...
    when(is_hit){
      sram(wayId).ports.cmd.payload.addr := cache_hit(wayId) ? cpu_bank_addr | U(0, bankDepthBits bits)
      sram(wayId).ports.cmd.valid        := cache_hit(wayId) & cpu.cmd.fire
