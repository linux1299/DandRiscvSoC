...
with ControlService
{
  ...
  override def build(pipeline: DandRiscvSimple): Unit = {
    ...
    if(hazard != null) pipeline plug new Area{
      ...
      hazard.rs1_from_wb  := writebackStage.arbitration.isValid && writebackStage.output(RD_WEN) && writebackStage.output(RD_ADDR)=/=U(0, 5 bits) && writebackStage.output(RD_ADDR)===execute.output(RS1_ADDR) && !(hazard.rs1_from_mem || hazard.rs1_from_load)
      hazard.rs2_from_wb  := writebackStage.arbitration.isValid && writebackStage.output(RD_WEN) && writebackStage.output(RD_ADDR)=/=U(0, 5 bits) && writebackStage.output(RD_ADDR)===execute.output(RS2_ADDR) && !(hazard.rs2_from_mem || hazard.rs2_from_load)
    ...
    fetch plug new Area{
      ...
      arbitration.flushIt := execute.output(REDIRECT_VALID) || output(INT_EN)
    }
    ...
    decode plug new Area{
      import decode._
      arbitration.haltItself := False
      arbitration.flushIt := execute.output(REDIRECT_VALID) || fetch.output(INT_EN)
    }
