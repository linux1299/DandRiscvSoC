...
case class Ifu(resetVector : BigInt = 0x80000000l, p : ICacheConfig) extends Component {
  import p._
  // ==================== IO =============================
  val flush = in Bool()
  val stall = in Bool()
  val interrupt_vld = in Bool()
  val icache = master(ICacheAccess(addressWidth, cpuDataWidth))

  val pc_next = Reg(UInt(addressWidth bits)) init(resetVector)
  ...
  val bpu_predict_taken = input(BPU_BRANCH_TAKEN) && icache.cmd.fire
  ...
  val fetchFSM = new Area{
    ...
    switch(fetch_state){
      ...
      is(FETCH) {
        when(icache.cmd.isStall || arbitration.isStuck){
      ...
      is(HALT){
        when(icache.cmd.ready && !arbitration.isStuck){
          fetch_state_next := FETCH
        }
        .otherwise{
          fetch_state_next := HALT
  ...
  pc_in_stream.valid   := icache.cmd.fire //all pc stream into FIFO
  ...
  predict_taken_in.valid := icache.cmd.fire
  ...
  instruction_in_stream.valid   := icache.rsp.valid && !rsp_flush && !fetch_flush
  instruction_in_stream.payload := icache.rsp.payload.data
  ...
  insert(PREDICT_VALID) := icache.cmd.fire // for BPU
  ...
  // send cmd to icache
  icache.cmd.valid := fetch_valid && !fetch_flush
  icache.cmd.addr  := pc_next.resized
